                ...
                .then((linkService) => {
                  ...
                  linkService.container_envvars.forEach((envObj) => {
                    if (envObj.key === 'VIRTUAL_HOST') {
                      ...
                      logger.info(linkService.container_ports);
                      let x;
                      for (x in linkService.container_ports) {
                        if (linkService.container_ports[x].inner_port === parseInt(envObj.value, 10)) {
                  ...
                  if (linkConfig.path && linkConfig.domain) {
                    ...
                    logger.error(`Missing ENV Variables: ${link.name}`, linkService.container_envvars, linkService.container_ports);
                    resolve();
                  }
                })
                .catch((err) => {
